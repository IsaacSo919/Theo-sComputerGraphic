...
def process_line(line):
    ...
    if fields[0] == '#':
        ...
        # Add a filter to handle empty strings
        return (label, [[np.uint32(i) for i in v.split('/') if i] for v in fields[1:]])
...
def fix_blender_textures(textures, faces, vertices):
    ...
    '''
    if faces.shape[2] == 1 or textures.size == 0:
        print('(W) No texture indices provided, setting texture coordinate array as None!')
        return None

    # Ensure textures array is two-dimensional
    if textures.ndim == 1:
        print('(W) Textures array is one-dimensional; setting texture coordinate array as None!')
        return None
